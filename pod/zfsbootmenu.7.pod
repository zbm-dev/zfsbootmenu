...
By default, ZFSBootMenu will look for the I<bootfs> property on the first pool it imports to select the default boot environment. If you have multiple pools, substitute the name of your preferred pool.
...
Set this option to only import pools that match the SPL hostid configured in ZFSBootMenu. If none can be imported, an emergency shell will be invoked.
...
Set this option to allow run-time reconfiguration of the SPL hostid. If a pool is preferred via B<zbm.prefer>, and the pool can not be imported, the I<hostid> used to last import the pool is discovered and set on the system. If a preferred pool is not set, attempt to discover the hostid of all available pools and import them.
