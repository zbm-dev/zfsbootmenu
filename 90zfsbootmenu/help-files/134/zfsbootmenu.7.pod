...
[1mzbm.tmux[0m
    Indicate that ZFSBootMenu should be run under [1mtmux[0m in the initramfs. With this enabled, debug-level logging for the ZFSBootMenu
    scripts can be easily viewed. The tooling must also be installed with an additional dracut configuration option listed below.
...
[1mzfsbootmenu_tmux=true[0m
    An optional variable enabling the installation of [1mtmux[0m and a minimal [1mtmux.conf[0m in the initramfs.

[1mzfsbootmenu_tmux_conf=<tmux.conf>[0m
    An optional variable specifying the path to an alternate [1mtmux[0m configuration file. If this key is set but no file exists at the
    path [1m<tmux.conf>[0m, the default configuration file is instead used.
