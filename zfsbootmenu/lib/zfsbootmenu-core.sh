...
load_key_clevis() {
  ...
  if [[ "$CLEVIS_CHECK" == "yes" ]]; then
    ...
    KEYLOCATION="$(get_fs_value "${dataset_for_clevis_unlock}" keylocation)"
    if [[ -z "$(echo "$KEYLOCATION"|grep file)" ]]; then
        # That's not us
        zdebug "Warning: keylocation is not file while clevis unlock is set for dataset $dataset_for_clevis_unlock"
        return 0
    fi
    ...
    if [[ "$KEYSTATUS" == "unavailable" ]]; then
        ...
        mkdir -p "$(dirname "$KEYLOCATION")"
        JWE="$(zfs get -H -p -o value latchset.clevis:jwe -s local $dataset_for_clevis_unlock)"
        echo "$JWE" | clevis decrypt >"$KEYLOCATION"
        cat "$KEYLOCATION" | zfs load-key -n $dataset_for_clevis_unlock
        if [[ "$?" == "0" ]]; then
          # We are fine
          return 0
        else
          ...
          if [[ "$RESEAL" != "" ]]; then
            ...
            if [[ "$ro_state" = "on" ]]; then
                zpool export "$pool"
                zpool import -o readonly=off "$pool"
            ...
            echo "$jwe_check" | clevis decrypt | zfs load-key -n -L prompt "$dataset_for_clevis_unlock"
            if [[ "$?" != "0" ]]; then
              # We failed. The jwe is not correctly stored in dataset
              zdebug "Warning: We failed. The jwe is not correctly stored in dataset $dataset_for_clevis_unlock"
              return 1
            fi
            echo "$RESEAL" >"$KEYLOCATION"
            return 0
          else
      ...
      zdebug "This is strange. Without our help the key is available. Probably keyfile was by mistake put into ZBM initramfs. Plese recheck ZBM configuration"
      return 1
    fi
  else
    #  That's not us
